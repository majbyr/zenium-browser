...
"""
Script to generate Zenium version numbers
...
def check_existing_version(path):
    ...
    with open(path, "r", encoding="utf-8") as file:
        if "ZENIUM" in file.read():
            raise ValueError("file already contains zenium versioning")
...
def get_version_parts(tree, platform_tree):
    ...
    version_paths = {
        "ZENIUM_MAJOR": tree / "version.txt",
        "ZENIUM_MINOR": tree / "chromium_version.txt",
        "ZENIUM_PATCH": tree / "revision.txt",
    }

    if platform_tree is not None:
        version_paths["ZENIUM_PLATFORM"] = platform_tree / "revision.txt"

    version_parts = {}
    for name, path in version_paths.items():
        delta = 0 if name != "ZENIUM_MINOR" else -CHROME_VERSION_BASE
